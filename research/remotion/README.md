...
- Colors look slightly different side by side. Remotion encodes Chrome's screenshots as full-range BT.601 (`yuvj420p`, `bt470bg`) by default, while the camera, Kdenlive, and the ffmpeg compiler use limited-range BT.709. `--color-space=bt709` would align it, and it does not matter for an editor preview.
